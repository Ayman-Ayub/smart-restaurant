...
export default function MultiStep (props) {
  ...
  const renderNav = (show) =>
    show && (
      <div className='flex flex-row gap-7'>
        <button
          style={buttonsState.showPreviousBtn ? {backgroundColor:'#f97316',paddingLeft:'20px',paddingRight:'20px',paddingTop:'5px',paddingBottom:'5px', fontSize:'20px' }: {display:"none"}}
          onClick={previous}
        >
          Prev
        </button>
        ...
        <button
          style={buttonsState.showNextBtn ? { backgroundColor:'#f97316',paddingLeft:'20px',paddingRight:'20px',paddingTop:'5px',paddingBottom:'5px', fontSize:'20px'} : { display:"none"}}
          onClick={next}
        >
          Next
        </button>
  ...
  return (
    <div style={{display: 'flex', flexDirection: directionType === 'column' ? 'row' : 'column'}}>
      ...
      <div className='pl-80 pb-40 pt-10'>{renderNav(showNav)}</div>
    </div>
  )
}
